...
- **UI fixes**: Restart the app → verify the fix (screenshot or browser automation) → compare with the broken state from step 1
- **Backend fixes**: test output is sufficient proof

### 7. Beautify (UI fixes only)

If the fix introduced new UI elements, refine the UI design.
